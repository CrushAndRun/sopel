...
def traffic(bot,trigger):
    ...
    if origin:
        ...
        if dest:
            url = 'https://maps.googleapis.com/maps/api/distancematrix/json?origins={0}&destinations={1}&avoid=tolls&departure_time=now&traffic_model=best_guess&units=metric&key={2}'.format(origin,dest,api)
            ...
            if result['rows'][0]['elements'][0]['status'] == 'NOT_FOUND':
                ...
                return bot.say("Duration to {0} [{1}]: {2} ({3} in traffic)".format(address,km,duration,traffic))
    else:
